...
def start_handler(message):
    ...
    if not is_subscribed(user_id):
        ...
        return  # يتوقف هنا حتى يشترك المستخدم
    ...
    if os.path.exists(WARN_FILE):
        ...
        if str(user_id) in data:
            try:
                ...
                pass  # إذا كانت الرسالة محذوفة مسبقاً

            del data[str(user_id)]
            with open(WARN_FILE, "w") as f:
                json.dump(data, f)

    # ✅ حفظ المستخدم بعد الاشتراك
    save_user(user_id)
...
# ✅ هذا هو الجزء الجديد
    if not has_been_verified(user_id):
        ...
        return    # ✅ إشعار الأدمن بدخول عضو جديد
       # ✅ التأكد من وجود مجلد البيانات
    if not os.path.exists("data"):
        os.makedirs("data")

    # ✅ قراءة المستخدمين المحفوظين
    if not os.path.exists(USERS_FILE):
        with open(USERS_FILE, "w") as f:
            f.write("")
    ...
    # ✅ إذا المستخدم جديد، نحفظه ونرسل إشعار
    if str(user_id) not in users:
